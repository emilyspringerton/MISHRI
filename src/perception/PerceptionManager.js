...
class PerceptionManager {
  ...
  async _lookLoop() {
    ...
    const nextDelay = this.h.config.lookWanderIntervalMin +
  ...
  async lookAtNearbyEntity() {
    ...
    await this.h.smoothTurn(
      this.bot,
      this.getYawTo(target.position),
      this.getPitchTo(target.position),
      this.h.randInt(5, 12)
    );
   ...
   */
  async lookAtInterestingBlock() {
    const pos = this.bot.entity.position;
    const radius = 8;
    const interestingBlocks = [
      'diamond_ore', 'iron_ore', 'gold_ore', 'emerald_ore',
      'redstone_ore', 'lapis_ore', 'flower', 'rose',
    ];
      ...
      for (let dy = -3; dy <= 3; dy += 2) {
        for (let dz = -radius; dz <= radius; dz += 2) {
          ...
          if (block && interestingBlocks.some((name) => block.name.includes(name))) {
            await this.h.smoothTurn(
              this.bot,
              this.getYawTo(block.position),
              this.getPitchTo(block.position),
              8
            );
            return;
          }
  ...
  async idleDrift() {
    ...
    const driftPitch = Math.max(-Math.PI / 2, Math.min(Math.PI / 2,
   ...
   */
  async onEntityHurt(entity) {
    if (entity === this.bot.entity) return;
    // Briefly look at whoever got hurt
    if (entity.position) {
      await this.h.smoothTurn(
        this.bot,
        this.getYawTo(entity.position),
        this.getPitchTo(entity.position),
        5
      );
    }
  }
   ...
   */
  getNearbyPlayerCount(radius = 32) {
